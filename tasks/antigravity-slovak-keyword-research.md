...
> **Уточнение 2026-09-26 («левая рука», по указанию владельца).** С
> языковой моделью v2 словацкие адреса уже живые и в индексе
> (`/sk/veterinar/bratislava/`, `/sk/podnik/{slug}/`, страницы цен
> `…/ceny/…`, признаков `…/sobota/`, `…/nedela/`, `…/nonstop/`,
> `…/exoticke-zvierata/`, `…/vyjazd-domov/`). Поэтому главный результат
> задачи — **словацкие `<title>`, H1 и формулировки** по реальным
> запросам. Замену слага рекомендовать, только если разница в спросе
> явная (каждая замена — 301 и потеря времени на переиндексацию).
> Проверить и слаги страниц цен и признаков.

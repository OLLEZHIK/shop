...
**Зависимости:** нет по другим задачам `tasks/`, **но есть блокер по
scaffold'у** — см. подраздел «Важно: `website/` ещё не в `main`» ниже,
прочитать перед стартом. Задачи `cli-build-listing-and-detail-pages` и
`cli-build-shell-and-static-pages` зависят от результата этой задачи —
начинать их после мерджа этой.

## Важно: `website/` ещё не в `main`

Проверено 2026-09-21 облачным Claude Code: каталога `website/` и
файла `DESIGN.md` в `main` сейчас **нет**. Весь исходный Next.js-scaffold
(из которого исходит эта задача и `docs/design-plan.md`) существует
только на отдельной ветке `cli/initial-website` (коммит "Initial
website implementation (Sprint 0 T0.1)", 2026-09-20) — по ней **не
было открыто ни одного PR**, она не ревьюжена и не смёржена. Эта ветка
к тому же устарела (сделана от более старого `main`, в её дифе видно
удаление уже заархивированных файлов `tasks/done/*`) — использовать её
как есть напрямую нельзя, нужен ребейз.

Перед началом работы над этой задачей:
1. Создать рабочую ветку `cli/generalize-business-model-and-seed` от
   актуального `main`.
2. Взять содержимое `website/` с `origin/cli/initial-website` (например,
   `git checkout origin/cli/initial-website -- website/` после ребейза
   на актуальный `main`, или `git merge`/`cherry-pick` — на усмотрение
   исполнителя, главное чтобы не потерялись уже смёрженные в `main`
   изменения типа архивации задач).
3. Дальше выполнять задачу как описано ниже, поверх этого scaffold'а.
4. В PR явно упомянуть, что `website/` scaffold внесён этой веткой
   (а не отдельным PR от `cli/initial-website`, который остаётся
   неиспользуемым и его можно будет закрыть/удалить отдельно).
